  ...
  if isinstance(sources, list):
    for i,s in enumerate(sources,1):
      if isinstance(s,dict):
        ...
        norm.append({"rank":i,"source":str(s),"score":None,"begin":None,"end":None})
  return {
...
def fallback_summary_from_docs(max_chars=1200):
  ...
  parts=[]
  import pypdf
  for pdf in pathlib.Path("data").rglob("*.pdf"):
    try:
      ...
      for i,pg in enumerate(r.pages[:3]):
        ...
        if sum(len(t) for t in text) > max_chars: break
      chunk = "\n".join(text).strip()
      if chunk:
        parts.append(f"[{pdf.name}] {chunk}")
  ...
  return "Fallback summary (first pages/snippets):\n" + "\n\n".join(parts)[:max_chars]

def answer_one(question:str):
  rc, out = run(f"python rag.py ask --store {shlex.quote(STORE)} --q {shlex
